...
class TestPage:
    ...
    def test_create_page(self, mocked: requests_mock.Mocker,
                         a_test_collab: collaboratory.Collab) -> bool:
        mocked.put(f'{COLLABORATORY_URL}/rest/wikis/xwiki/spaces/Collabs/spaces/{COLLAB_NAME}'
                   + '/pages/WebHome/attachments/test.py')
        a_test_collab.attach(name='test.py', content="print('hello')")
        assert (mocked.last_request.url ==
